...
static size_t telemetry_frequency_in_seconds = 1; // With default frequency of once in 1 seconds.
...
/* --- Internal Functions --- */
const int ldrPin1 = 17;  // Pin LDR
const int trigPin = 19;  // Pin trig untuk ultrasonik
const int echoPin = 21;   // Pin echo untuk ultrasonik



// Fungsi untuk membaca nilai LDR
static float get_ldr_value() {
  return analogRead(ldrPin1);
}
...
static void get_ultrasonic_values(float* Distance) {
  ...
  *Distance = duration * 0.034 / 2; // Konversi durasi ke jarak dalam cm
}
    ...
    size_t* payload_buffer_length)
{
  ...
  // Tambahkan data jarak ultrasonik
